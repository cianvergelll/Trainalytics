<script>
	...
	async function fetchStudentJournals(page) {
		...
		const cacheKey = `student_journals_${studentId}?${params.toString()}`;
		...
		try {
			...
			if (res.ok) {
				...
				error = err.error || 'Failed to load journal logs.';
			}
		} catch (e) {
			console.error(e);
	...
	onMount(() => {
		fetchStudentJournals(1);
	});

	function changePage(newPage) {
		if (newPage < 1 || newPage > totalPages) return;
		fetchStudentJournals(newPage);
	}
	...
	function handleViewJournal(id) {
		goto(`/admin/main/journals/view?id=${id}&from=student-list&studentId=${studentId}`);
	}
		...
		<div class="flex-1 overflow-y-auto p-8">
			<div class="mx-auto flex max-w-[1600px] flex-col gap-6">
				<div class="flex items-center justify-between">
					<div>
						<h1 class="text-3xl font-bold text-gray-800">Student Journal Logs</h1>
						<p class="mt-1 text-sm text-gray-500">
							Pages / Journals / {studentInfo.name || 'Student'} / Logs
						</p>
					</div>
					<button
						onclick={handleBack}
						...
						<div class="overflow-hidden rounded-lg border border-gray-200">
							<table class="min-w-full divide-y divide-gray-200">
								<thead class="bg-gray-50">
									<tr>
										<th
											class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-600 uppercase"
											>Date</th
										>
										<th
											class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-600 uppercase"
								...
								<tbody class="divide-y divide-gray-200 bg-white">
									...
									{:else}
										{#each journalLogs as log}
											<tr class="transition-colors hover:bg-gray-50">
												<td class="px-6 py-4 text-sm whitespace-nowrap text-gray-800">{log.Date}</td
												>
												...
												<td class="px-6 py-4 text-sm whitespace-nowrap text-gray-800">
													<div class="flex items-center gap-3">
														<button
															onclick={() => handleViewJournal(log.ID)}
