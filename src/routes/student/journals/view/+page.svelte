	...
	<div class="flex h-full flex-1 flex-col overflow-hidden rounded-xl bg-white p-6 shadow-lg">
		<div class="mb-4 flex flex-shrink-0 items-center justify-between">
			<div class="flex items-center gap-3">
				...
				<h1 class="text-xl font-bold text-gray-800">My Journal Entry</h1>
			</div>
		</div>

		{#if loading}
			<div class="flex h-full items-center justify-center text-gray-500">Loading entry...</div>
		{:else if journal}
			<div class="grid min-h-0 flex-1 grid-cols-1 gap-6 overflow-hidden lg:grid-cols-3">
				<div class="overflow-y-auto pr-2 lg:col-span-2">
					<div class="mb-4 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
						<div class="mb-6 flex items-start justify-between border-b border-gray-100 pb-6">
							<div>
								<h2 class="text-lg font-bold text-gray-900">{journal.taskName}</h2>
								<p class="text-sm text-gray-500">{journal.company}</p>
							</div>
				...
				>
					...
					<div bind:this={chatContainer} class="flex-1 space-y-4 overflow-y-auto bg-gray-50/50 p-4">
						{#if chatHistory.length === 0}
							<div class="mt-10 text-center text-sm text-gray-400">No feedback messages yet.</div>
						{/if}

						{#each chatHistory as msg}
							<div class={`flex ${msg.SenderType === 'STUDENT' ? 'justify-end' : 'justify-start'}`}>
								<div
								...
								>
									...
									<span
										class={`mt-1 block text-[10px] ${
											msg.SenderType === 'STUDENT' ? 'text-right text-blue-100' : 'text-gray-400'
										}`}
									>
					...
					<div class="border-t bg-white p-3">
						...
						>
							<input
								...
								placeholder="Reply to coordinator..."
